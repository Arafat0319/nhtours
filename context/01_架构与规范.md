...
| 用户确认 | 用户明确表示「可以 push / 部署」后，再 `git push origin main` | 默认自动部署 |

- 用户仅要求改代码、未提部署时：**只改代码（或留工作区待用户 commit），不 push**。
- Cursor 持久规则：**`.cursor/rules/local-test-before-push.mdc`**（`alwaysApply: true`）。
...
**最后更新**: 2026-06-25（本地测试后再 push 规则）
