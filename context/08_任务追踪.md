...
| 任务 | 状态 | 说明 |
|------|------|------|
| `/home-preview` 实验页 | ✅ 完成 | Modern V1：Hero、Tagline、Apart、Testimonials、Programs、Newsletter |
| 独立实验 navbar | ✅ 完成 | `includes/home_preview/nav.html` + `home-preview-nav.js` |
| Programs 卡片徽章 | ✅ 完成 | Popular（琥珀）/ NEW（蓝色） |
| 合并至正式首页 `/` | ⏳ 待验收 | 验收通过后替换 `index.html`，并决定 navbar 策略 |
| Our Team 正式页 | ✅ 完成 | `/our-team`：Hero 52vh、无 indigo 条、4 列成员；`body.our-team-page` CSS 隔离；footer 已链接 |
| Asia Family Photo Gallery | ✅ 完成 | Photo 1 补全为 `photo-1.png`（`ed56b97`）；避免 Unicode 文件名 |
...
| 任务 | 说明 | 预计工作量 |
|------|------|-----------|
| 首页实验页合并至 `/` | `/home-preview` 已定稿，验收后合并 layout/样式至正式首页 | 中 |
...
**最后更新**: 2026-06-14（Asia Family Photo 1 修复、Deploy 查看与部署排查）
