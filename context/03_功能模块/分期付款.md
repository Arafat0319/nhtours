...
> **字段名以 `models.py` 为准**：`InstallmentPayment.payment_intent_id`（非 `stripe_payment_intent_id`）；`Payment` 模型无 `payment_type` 列——类型信息在 Stripe metadata 或业务逻辑中区分。
...
### 后续分期支付（强制补齐 Catch-up）

打开任一未付期链接时，应付金额 = **该期 + 此前所有 pending/overdue**（按 `installment_number`）。  
一笔 Stripe 扣款 → 一行 Payment → 一张收据；被覆盖的较早分期一并标 `paid`（schedule 可注 *Included in catch-up payment*）。

```
客户点击邮件中的 /pay-installment/<id>
    ↓
catch_up = 未付且 number ≤ 锚定期
应付 = sum(catch_up)；summary 多行；metadata.catch_up_ids
    ↓
POST /api/payment/quote|intent（installment_id；金额为合计）
    ↓
支付成功 → Webhook
    ↓
覆盖集合内分期全部 paid；只建一笔 Payment
void 其他 pending PI；必要时 cancel 剩余未付（全款结清时）
```

Payoff 仍为一次付清整单剩余（取消其余未付分期），与 catch-up 并存。
...
> **邮件**：生产 SES 已配置；催款链接依赖 **`BASE_URL=https://nhtours.com`**（`tasks.py`）。排障见 `06` SES 节。
...
- **分期付款弹窗页**：`installment_modal_page.html`（body class: `installment-modal-page`），布局与报名弹窗一致（左 PAYMENT / 右 Your Booking），共用 `_modal_payment_step_content.html`、`booking-modal.css`；圆图压卡片上缘（一半在框外）；Confirm 按钮与报名页 nextBtn 风格一致。
- **导航**：分期页使用 `includes/nav_booking.html`，且通过 CSS 隐藏 `#pageNav`（`display: none`），整页为弹窗形态。
- **移动端**：`#booking-modal-scroll-viewport` 在 &lt;1024px 下设为 `max-height: 100vh`、`overflow-y: auto`，弹窗内可滚动以看到 Your Booking。
- **文案**：界面中「Pay Off」为全大写「PAY OFF」。
- **根模板**：`base.html` 需包含 `{% block extra_head %}{% endblock %}`，分期页通过该块注入样式与 `booking-modal.css`，否则分期页专用样式不生效。
