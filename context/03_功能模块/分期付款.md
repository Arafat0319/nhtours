...
> **⚠️ 上线前提醒**（2026-06-30 域名接入后）：自动/手动提醒邮件中的支付链接须生产环境配置 **`BASE_URL=https://nhtours.com`**（`tasks.py`）；SES 域名验证 DKIM 须在 **GoDaddy DNS** 添加。实现或测试邮件前，先读 **`08_任务追踪.md` →「邮件/SES 上线前必读」**。
